...
if(isset($_POST['btn-lupa']))
{
	$fpemail = aman($_POST['fpemail']);
	
	$stmt = $user_home->runQuery("SELECT * FROM login, member WHERE login.email=:email AND member.email=:email LIMIT 1");
	$stmt->execute(array(":email"=>$fpemail));
	$forgot= $stmt->fetch(PDO::FETCH_ASSOC);
	$username_mail = $forgot['nama_member'];
	...
	if($stmt->rowCount() == 1)
	{
		...
		$stmt = $user_home->runQuery("UPDATE login SET passkey=:passkey WHERE email=:email");
...
$headers .= 'Reply-To: '.$username_mail.'<'.$fpemail.'>'. "\r\n";
...
<table class="table-row" width="450" bgcolor="#FFFFFF" style="table-layout: fixed; background-color: #ffffff;" cellspacing="0" cellpadding="0" border="0"><tbody><tr><td class="table-row-td" style="font-family: Arial, sans-serif; line-height: 19px; color: #444444; font-size: 13px; font-weight: normal; padding-left: 36px; padding-right: 36px;" valign="top" align="left">
  <table class="table-col" align="left" width="378" cellspacing="0" cellpadding="0" border="0" style="table-layout: fixed;"><tbody><tr><td class="table-col-td" width="378" style="font-family: Arial, sans-serif; line-height: 19px; color: #444444; font-size: 13px; font-weight: normal; width: 378px;" valign="top" align="left">
     <div style="font-family: Arial, sans-serif; line-height: 19px; color: #444444; font-size: 13px; text-align: center;">
      <a href="http://ayoklarifikasi.com/home.html?user='.$id.'&passkey='.base64_encode($passkey).'#reset" style="color: #ffffff; text-decoration: none; margin: 0px; text-align: center; vertical-align: baseline; border: 4px solid #6fb3e0; padding: 4px 9px; font-size: 15px; line-height: 21px; background-color: #6fb3e0;">&nbsp; Ganti Password &nbsp;</a>
    </div>
...
$send = mail($fpemail,"Lupa Kata Sandi", $pesan, $headers);

			echo "Lupa Kata Sandi Berhasil Dilakukan, Silahkan Cek Inbox atau Spam Email Kamu";
					exit;
		}
		else
		{
			echo "Lupa Kata Sandi Gagal Dilakukan";
					exit;
		}
}
